...
- Produktdefault und Zoom-Floor werden deterministisch getestet: 24 px für Knoten, 22 px für Kanten; der interne 18/16-px-Konverter-Fallback bleibt unverändert; bei einem 40-%-Fit folgen genau drei 1,2-fache Zoomschritte; zusätzlich werden die 2-px-Differenz, linksbündige Erzeugungsdefaults nur für neue/importierte Diagramme, die unveränderte Behandlung bestehenden draw.io/XMLs und gespeicherter Entwürfe, 8–72-px-Präferenzgrenzen, ein abweichender JSON-Canvas-Schriftstandard und die nativen Schrift-Aktionen (`decreaseFontSize`, `increaseFontSize`, `format`, `selectAll`) geprüft;
